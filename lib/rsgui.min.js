...
RSGUI.prototype.constructor=RSGUI;RSGUI.prototype.loadTheme=function(url){var self=this;self.add.waitTheme();var xobj=new XMLHttpRequest();xobj.overrideMimeType("application/json");xobj.open('GET',url,true);xobj.onreadystatechange=function(){if(xobj.readyState==4&&xobj.status=="200"){theme=JSON.parse(xobj.responseText);theme.path=url.substring(0,url.lastIndexOf("/")+1);self.add.setTheme(theme);}};xobj.send(null);}
RSGUI.prototype.loadAsset=function(){this.add.loadAsset();}
GUIFactory=function(game){this.game=game;this.theme=this.getDefaultTheme();this._theme_ready=true;this.queue=[];}
GUIFactory.prototype.constructor=GUIFactory;GUIFactory.prototype={window:function(x,y,width,height,title,container){var object=new Window(this.game,x,y,width,height,title);this.addToGroup(container,object);this.setObjectTheme(object,'window');return object;},listview:function(x,y,width,height,container){var object=new ListView(this.game,x,y,width,height);this.addToGroup(container,object);this.setObjectTheme(object,'listview');return object;},button:function(x,y,text,container){var object=new Button(this.game,x,y,text);this.addToGroup(container,object);this.setObjectTheme(object,'button');return object;},checkbox:function(x,y,text,container){var object=new CheckBox(this.game,x,y,text);this.addToGroup(container,object);this.setObjectTheme(object,'checkbox');return object;},label:function(x,y,text,container){var object=new Phaser.Text(this.game,x,y,text);this.addToGroup(container,object);return object;},textinput:function(x,y,text,container){var object=new TextInput(this.game,x,y,text);this.addToGroup(container,object);this.setObjectTheme(object,'textinput');return object;},rating:function(x,y,stars,container){var object=new Rating(this.game,x,y,stars);this.addToGroup(container,object);this.setObjectTheme(object,'rating');return object;},scrollbar:function(x,y,width,height,type,container){var object=new ScrollBar(this.game,x,y,width,height,type);this.addToGroup(container,object);this.setObjectTheme(object,'scrollbar');return object;},processbar:function(x,y,width,height,type,container){var object=new ProcessBar(this.game,x,y,width,height,type);this.addToGroup(container,object);this.setObjectTheme(object,'processbar');return object;},dropdown:function(x,y,text,container){var object=new DropDown(this.game,x,y,text);this.addToGroup(container,object);this.setObjectTheme(object,'dropdown');return object;},radiobox:function(x,y,text,container){var object=new RadioBox(this.game,x,y,text);this.addToGroup(container,object);this.setObjectTheme(object,'radiobox');return object;},toggle:function(x,y,text,container){var object=new ToggleButton(this.game,x,y);this.addToGroup(container,object);this.setObjectTheme(object,'toggle');return object;},tooltip:function(x,y,text,container){var object=new ToolTip(this.game,x,y,text);this.addToGroup(container,object);this.setObjectTheme(object,'tooltip');return object;},addToGroup:function(container,object){if(container==null)
this.game.world.add(object);else
container.addChild(object);},setFont:function(font){},setTheme:function(theme){var self=this;this.loadWebFont(theme.webfont,function(){self.loadAsset(theme);});},waitTheme:function(){this._theme_ready=false;},themeReady:function(theme){this._theme_ready=true;for(key in theme)
this.theme[key]=theme[key];for(var i=0;i<this.queue.length;i++){this.queue[i].obj.setTheme(this.theme[this.queue[i].type]);}
this.queue=[];},setObjectTheme:function(object,type){object.setTheme(this.theme[type]);if(!this._theme_ready){this.queue.push({obj:object,type:type});}},loadWebFont:function(fonts,activeCallback){var self=this;if(fonts==null){activeCallback();}else{webfontConfig={};if(fonts.google!=null)webfontConfig.google=fonts.google;if(fonts.custom!=null)webfontConfig.custom=fonts.custom;webfontConfig.active=activeCallback;WebFont.load(webfontConfig);}},loadAsset:function(theme){var loader=new Phaser.Loader(this.game);for(component in theme){if(typeof(theme[component])!='object')
continue;if('texture'in theme[component]){texture=theme[component]['texture'];for(key in texture){assetkey='rsgui-'+component+'-'+key;path=theme.path+texture[key];loader.image(assetkey,path);}}}
loader.onLoadComplete.addOnce(this.themeReady,this,0,theme);loader.start();}}
...
data=property.match(/\S+/g);for(i=0;i<data.length;i++){if(data[i].indexOf('#')!=-1)
...
GUIObject.prototype.onInputDownHandler.call(this,sprite,pointer);};ToggleButton.prototype.getType=function(){return'toggle';}
...
CheckBox=function(game,x,y,text){GUIObject.call(this,game,x,y);this._seprate=2;this._text=text;this._check=false;this._onFrame="rsgui-checkbox-on";this._offFrame="rsgui-checkbox-off";this._frame='on';};CheckBox.prototype=Object.create(GUIObject.prototype);CheckBox.prototype.constructor=CheckBox;CheckBox.prototype.draw=function(){var b=this._border;var r=this._radius;var w=this.width/this.scale.x;var h=this.height/this.scale.y;var c=h+this._seprate;var fontcolor=this._font.color;var font=this.getFont();this._bmd.cls();this._bmd.ctx.strokeStyle=this._borderColor;if(!this._hasTexture){this._bmd.ctx.fillStyle=this._color;this._bmd.ctx.roundRect(b+r,b+r,h-2*r-2*b,h-2*r-2*b,r,true);this._bmd.ctx.fill();if(this._check){this._bmd.ctx.fillStyle=this._checkColor;this._bmd.ctx.beginPath();this._bmd.ctx.arc(h/2,h/2,h/4,0,2*Math.PI);this._bmd.ctx.closePath();this._bmd.ctx.fill();}}else{var texture=this._texture[this._frame];this._bmd.copy(texture.key,0,0,texture.width,texture.height,0,0,h,h);}
...
CheckBox.prototype.check=function(){this._check=true;this._frame='on';}
CheckBox.prototype.uncheck=function(){this._check=false;this._frame='off';}
...
RadioBox=function(game,x,y,text){GUIObject.call(this,game,x,y);this._seprate=2;this._text=text;this._check=false;this._onFrame="rsgui-checkbox-on";this._offFrame="rsgui-checkbox-off";this._frame='off';this.onChange=new Phaser.Signal();};RadioBox.prototype=Object.create(GUIObject.prototype);RadioBox.prototype.constructor=CheckBox;RadioBox.prototype.draw=function(){var b=this._border;var r=this._radius;var w=this._originWidth;var h=this._originHeight;var c=h+this._seprate;var fontcolor=this._font.color;var font=this.getFont();this._bmd.cls();this._bmd.ctx.strokeStyle=this._borderColor;if(!this._hasTexture){this._bmd.ctx.fillStyle=this._color;this._bmd.ctx.arc(h/2,h/2,h/3,0,2*Math.PI);this._bmd.ctx.fill();if(this._check){this._bmd.ctx.fillStyle=this._checkColor;this._bmd.ctx.beginPath();this._bmd.ctx.arc(h/2,h/2,h/5,0,2*Math.PI);this._bmd.ctx.closePath();this._bmd.ctx.fill();}}else{var texture=this._texture[this._frame];this._bmd.copy(texture.key,0,0,texture.width,texture.height,0,0,h,h);}
this._bmd.ctx.font=font;this._bmd.ctx.fillStyle=fontcolor;this._bmd.ctx.textBaseline="middle"
this._bmd.ctx.fillText(this._text,c,h/2);}
...
RadioBox.prototype.check=function(){this._check=true;this._frame='on';if(this.parent!=null){for(i=0;i<this.parent.children.length;i++){var child=this.parent.children[i];if(child!=this&&child.group==this.group){if(child instanceof RadioBox)child.uncheck();}}}}
RadioBox.prototype.uncheck=function(){this._check=false;this._frame='off';}
...
Button=function(game,x,y,text){GUIObject.call(this,game,x,y);this._text=text;this._upFrame='rsgui-button-up';this._downFrame='rsgui-button-down';this._frame='up';this._upColor='#ccc';this._downColor='#333';};Button.prototype=Object.create(GUIObject.prototype);Button.prototype.constructor=Button;Button.prototype.draw=function(){var b=this._border;var r=this._radius;var w=this._originWidth;var h=this._originHeight;var fontcolor=this._font.color;var font=this.getFont();this._bmd.cls();this._bmd.ctx.lineWidth=b;this._bmd.ctx.strokeStyle=this._borderColor;if(!this._hasTexture){if(this._state=='down'){this._bmd.ctx.fillStyle=this._downColor;}else{var my_gradient=this._bmd.ctx.createLinearGradient(0,0,0,50);my_gradient.addColorStop(0,this._upColor);my_gradient.addColorStop(1,this._downColor);this._bmd.ctx.fillStyle=my_gradient;}
this._bmd.ctx.roundRect(b,b,w-2*b,h-2*b,r,true);this._bmd.ctx.fill();this._bmd.ctx.strokeBorder(b);}else{var texture=this._texture[this._frame];this._bmd.generateNinePatchTexture(texture.key,0,0,w,h,r,texture.width,texture.height);}
this._bmd.ctx.fillStyle=fontcolor;this._bmd.ctx.font=font;this._bmd.ctx.textBaseline='top';this._bmd.ctx.fillText(this._text,b+r,b+r);};Button.prototype.getType=function(){return'button';}
Button.prototype.onInputDownHandler=function(sprite,pointer){this._frame='down';GUIObject.prototype.onInputDownHandler.call(this,sprite,pointer);};Button.prototype.onInputUpHandler=function(sprite,pointer,isOver){this._frame='up';GUIObject.prototype.onInputUpHandler.call(this,sprite,pointer);};Button.prototype.setTheme=function(theme){GUIObject.prototype.setTheme.call(this,theme);this._upColor=theme.up;this._downColor=theme.down;}
...
TextInput.prototype.getValue=function(){return this._text;}
