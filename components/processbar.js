	...
	if(!this._hasTexture){
		...
		if(p>0){
			...
			if(this._scrolltype=='vertical'){
				this._bmd.ctx.roundRect(b+r/4, b+r/4, w-2*b-r/2, p+b+r/4, r/2, true);
			}else{
				this._bmd.ctx.roundRect(b+r/4, b+r/4, p-b-r/4, h-2*b-r/2, r/2, true);
			}
			this._bmd.ctx.fill();
		}
	}else{
...
ProcessBar.prototype.getValue=function(){
	if(this._scrolltype=='vertical')
		maxposition=this.height-this._radius/2-this._border;
	else
